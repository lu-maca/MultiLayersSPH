...
contains
      ...
      do i = 1,nn
        if(p(k)%V(i).ne.0d0) then
          S = g*sin(beta)*p(k)%Have(i)+&
              ...
              &(nu+p(k)%nu_t(i))*(p(k)%dudzplus(i)-p(k)%dudzmin(i))
          A = 0
          do j =1,nn
  ...
  subroutine turbulent_viscosity(p,gr)
    implicit none
    integer :: i,k,loc,nn
    ...
    do k =1,nlayer
      ...
      do i=1,nn
        if (p(k)%V(i).ne. 0d0) then
          ...
          p(k)%nu_t(i) = Cs**2*Delta**2*dudz*sqrt(2.)
...
! ------------------- GRADIENT CORRECTION ---------------------------------
  ...
  subroutine set_particles_OPEN(p,bp)
    ...
    do k = 1,nlayer
      ...
      allocate(p(k)%dhdxplus(np+20),p(k)%dhdxmin(np+20),p(k)%nu_t(np+20))
