    ...
    do k =1,nlayer
      ...
      do i = 1,mm
        if(bp(k)%x(i).lt.domain(1).and.bp(k)%V(i).ne.0.) then
          ...
          bp(k)%u(i) =  inusx(k)!*(1d0 + 0.2d0*sin(2d0*pi*dble(steptime)*dt))!gr(k)%u(idx_init_dom)
          if(steptime*dt >= 0.05d0) bp(k)%u(i) =  inusx(k)*(1d0 + 0.2d0*sin(2d0*pi*dble(steptime)*dt))
           ! if (steptime*dt >= 0.2d0 .and.steptime*dt <= 0.29d0)then !.or. steptime*dt >= 1d0 .and.steptime*dt <= 1.1d0 &
               ! & .or. steptime*dt >= 1.5d0 .and.steptime*dt <= 1.6d0 .or. steptime*dt >= 2d0 .and.steptime*dt <= 2.1d0 &
               ! & .or. steptime*dt >= 2.5d0 .and.steptime*dt <= 2.6d0 .or. steptime*dt >= 3d0 .and.steptime*dt <= 3.1d0 ) then
             !  bp(k)%u(i) =  inusx(k)*1.25
           ! endif
