...
contains
  ...
  subroutine derivatives(gr)
      ...
      !if (  steptime == 501) print*,gr(1)%dhdx
      ...
      do i = 1,ninterp
        ...
        do k =0,nlayer-1
          if(k.EQ.0) then
         		gr(k)%dudz(i) = 2d0*uave(k+1)/gr(k+1)%have(i)
  ...
  subroutine init_grid_OPEN(gr)
    ...
    do i = 1,ninterp
       if(gr(1)%x(i).ge.domain(1)) then
         idx_init_dom = i
         print*, idx_init_dom,gr(1)%x(idx_init_dom)
         exit
       endif
     end do
     do i = ninterp,1,-1
       if(gr(1)%x(i).le.domain(2)) then
         idx_end_dom = i
         print*, idx_end_dom, gr(1)%x(idx_end_dom)
         exit
       endif
     end do
    ...
    do k = 1,nlayer
      ...
      do i = 1,nn
        if(p(k)%V(i).ne.0.) then
          ...
          !if ( k==1.and. steptime == 501) print*,p(k)%dhdxplus(i),p(k)%loc(i), p(k)%x(i)
        end if
      end do
      ...
      nn = size(p(k)%x)
        do i = 1,nn
          if (p(k)%V(i).ne.0d0) then
            valore = p(k)%x(i)
            do l = 1,ninterp
              if ( abs(valore-gr(1)%x(l)).le. valstep .and. valore.eq.domain(2)) then
                p(k)%loc(i) = l-1
    ...
    do k = 1,nlayer
      ...
      do i =1,ninterp
        if (gr(k)%x(i).ge.domain(1).and.gr(k)%x(i).le.domain(2)) then
          ...
          do m = 1,nn
              R = gr(k)%x(i) - p(k)%x(m)
              if (ABS(R) .LE. 2d0*gr(k)%l(i).and. p(k)%V(m).ne.0.) THEN
          ...
          do m = 1, mm
              ...
              if (ABS(Rbc).LE.2d0*gr(k)%l(i).and. bp(k)%V(m).ne.0.) THEN
