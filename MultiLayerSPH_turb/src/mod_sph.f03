...
contains
      ...
      do i = 1,nn
        if(p(k)%V(i).ne.0.) then
          S = g*sin(beta)*p(k)%Have(i)+&
              &1./rho*(p(k)%pplus(i)*p(k)%dhdxplus(i)- p(k)%pmin(i)*p(k)%dhdxmin(i))+&
              &p(k)%nu_t(i)*(p(k)%dudzplus(i)-p(k)%dudzmin(i))
          A = 0
          do j =1,nn
            if(p(k)%V(j).ne.0.) then
              ...
              if(abs(R).le.2.*p(k)%l(i)) then
                  ...
                  if(n4.GE.0) then
                      ...
                      n1 = 0.5*(p(k)%l(i)+p(k)%l(j))
                      n2 = 0.5*(p(k)%Have(i) + p(k)%Have(j))
                      n3 = 0.5*(sqrt(g*p(k)%Have(i)) + sqrt(g*p(k)%Have(j)))
                      visc = -art_visc_coeff*n1*n2*n3*n4/(p(k)%x(i)-p(k)%x(j))**2
                  end if
                  bracksx = 1/rho*(p(k)%Have(i))*p(k)%Pave(i) + 0.5*visc
                  brackdx = 1/rho*(p(k)%Have(j))*p(k)%Pave(j) + 0.5*visc
            ...
            if(bp(k)%V(j).ne.0.) then
              ...
              if(abs(R).le.2.*p(k)%l(i)) then
                  ...
                  if (n4.ge.0) then
                      ...
                      n1 = 0.5*(p(k)%l(i) + bp(k)%l(j))
                      n2 = 0.5*(p(k)%Have(i) + bp(k)%Have(j))
                      n3 = 0.5*(sqrt(g*p(k)%Have(i)) + sqrt(g*bp(k)%Have(j)))
                      visc = -art_visc_coeff*n1*n2*n3*n4/(p(k)%x(i)-bp(k)%x(j))**2
                  end if
                  bracksx = 1/rho*p(k)%Have(i)*p(k)%Pave(i) + 0.5*visc
                  brackdx = 1/rho*bp(k)%Have(j)*bp(k)%Pave(j) + 0.5*visc
                  A = A + bp(k)%w(j)*(bracksx*p(k)%B(i)*dWw(R,p(k)%l(i)) + brackdx*bp(k)%B(j)*dWw(R,bp(k)%l(j)))
              end if
            end if
          end do
          p(k)%a(i) = 1/p(k)%Have(i)*(S-A)

          if (abs(p(k)%a(i)) .ge. huge(1.)) then
            print*,'acceleration inf',p(1)%have(i),S,A,p(1)%x(i),p(1)%xold(i)
            stop
          endif
        end if
      end do
  ...
  subroutine secant(p,bp)
    IMPLICIT NONE
      ...
      DOUBLE PRECISION :: sommaold,sommaoold,eps,toll,r,oldl
      ...
      DO k =1,nlayer
        ...
        DO i = 1,nn
          ...
          ! secant method
            ...
            loold = p(k)%l(i)
            oldl = loold
            old = p(k)%have(i)*(1. + 0.1)
            lold = p(k)%inl(i)*p(k)%inhave(i)/old
            foold = 0
            fold = 0
            DO j = 1,nn
              r = p(k)%x(i)-p(k)%x(j)
              IF (abs(r).le.2.*p(k)%l(i).and. p(k)%V(j).ne.0.) then
            ...
            DO j = 1,mm
              r = p(k)%x(i)-bp(k)%x(j)
              IF (abs(r).le.2.*p(k)%l(i).and. bp(k)%V(j).ne.0.) then
            ...
            DO WHILE (abs(toll).ge. eps)
              ...
              p(k)%have(i) = old
              p(k)%l(i) = lold
             ! if (p(k)%l(i).ge.huge(1.)) p(k)%l(i)=oldl
  ...
  subroutine turb_visc(p,gr)
    implicit none
    integer :: i,k,loc
    real*8 :: lm,pos, Cs,dudz, Delta
    TYPE(particles), DIMENSION(:), ALLOCATABLE :: p
    ...
    do k =1,nlayer
      do i=1,np
        ...
        !lm = gr(nlayer)%zcoord(loc)*0.01
        !p(k)%nu_t(i) = lm*lm*0.5*abs(p(k)%dudzmin(i)+p(k)%dudzplus(i))  + nu

        ! Smagorinsky model
        Cs = 0.2d0
        dudz = 0.5* abs(p(k)%dudzmin(i)+p(k)%dudzplus(i))
        Delta = gr(nlayer)%zcoord(loc)/nlayer
        p(k)%nu_t(i) = Cs**2*Delta**2*dudz*sqrt(2.)+nu
      end do
    !  if (k.eq.1 .and. mod(steptime,20).eq.0) print*,p(k)%nu_t
    end do
...
! ------------------- GRADIENT CORRECTION ---------------------------------
  ...
  SUBROUTINE set_particles_CLOSE(p,bp)
    ...
    DO k = 1, nlayer
      ...
      ALLOCATE(p(k)%dhdxplus(Np),p(k)%dhdxmin(Np),p(k)%inhave(np),p(k)%inl(np),p(k)%nu_T(Np))
    ...
    DO k = 1,nlayer
      ...
      case (2) ! dam break with wet bed
        htot = 0.4 ! left state with h_L > h_R
        hlayer = htot/nlayer
  ...
  subroutine set_particles_OPEN(p,bp)
    ...
    do k = 1,nlayer
      np = p(k)%np
      ! fluid particles
      allocate(p(k)%x(np+35),p(k)%u(np+35),p(k)%a(np+35))
      allocate(p(k)%V(np+35),p(k)%w(np+35),p(k)%have(np+35),p(k)%l(np+35))
      allocate(p(k)%B(np+35),p(k)%inhave(np+35),p(k)%inl(np+35))
      allocate(p(k)%xold(np+35),p(k)%uold(np+35),p(k)%aold(np+35))
      allocate(p(k)%pave(np+35),p(k)%pplus(np+35),p(k)%pmin(np+35))
      allocate(p(k)%dudzplus(np+35),p(k)%dudzmin(np+35),p(k)%loc(np+35))
      allocate(p(k)%dhdxplus(np+35),p(k)%dhdxmin(np+35))
    ...
    do k =1,nlayer
        do i = 1,np
          ...
          case (3) ! nusselt
            ...
            p(k)%u(i) =  .5d0*g/nu*(2*htot-(dble(k)-0.5)*hlayer)*(dble(k)-0.5)*hlayer*sin(beta)!0.1*dble(k)!
            p(k)%have(i) = p(k)%V(i)/p(k)%w(i)
            p(k)%inhave(i) = p(k)%Have(i)
            p(k)%inl(i) = p(k)%l(i)
          case (4)
...
!--------------------------------------------------------------------
  ...
  subroutine output(gr,p,bp)
    ...
    do i = 1,ninterp
      write(1,*) gr(1)%x(i), gr(nlayer)%zcoord(i)
    end do
    close(1)
  ...
  subroutine stability(p,gr)
    ...
    if (maxval(cfl).ge.1 .or. dt * 1./minval(diff_stab).ge.0.5 ) then
      if (testcase.ne. 1) then
        ...
        print*, p(k)%l
        print*, maxloc(cfl)
        stop ! the front wave can give strange behaviour for the diffusion stability
      end if
    endif
